        ...
        <div className='absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 max-w-xl w-full bg-black/75 rounded-md p-18 mt-10 flex flex-col justify-center items-center shadow-md space-y-6'>
            ...
            <form className='space-y-6 w-full' onSubmit={submit}>
                {/* <div className='group px-5 py-2 border border-gray-600 rounded-md relative focus-within:ring-1 focus-within:ring-white tabindex="0"' >
                    <label htmlFor='email' className='text-3xl  text-gray-400 absolute top-5 left-5 group-hover:text-lg group-hover:top-2 transition-all' >Email</label>
                    <input type='input' name='email' id="email" className='mt-8 w-full text-white text-xl focus:outline-none '></input>
                </div> */}
                ...
                <div className='tabindex="0" px-5 py-2 border border-gray-600 bg-white/3 rounded-md relative focus-within:ring-1 focus-within:ring-white' style={{cursor: 'text'}} onFocus={()=> setIsUsernameFocused(true)} onBlur={()=> {formData.username? null:setIsUsernameFocused(false)}}>
                    <label htmlFor='username' className={`text-gray-400 absolute font-extralight left-5 ${isUsernameFocused? "text-lg top-2":"text-2xl top-6"} transition-all`} >Username</label>
                    <input type='input' name='username' id="username" className='mt-8 w-full text-white text-xl focus:outline-none' onChange={(event) => {setFormData((pre)=>{return {...pre, [event.target.name] : event.target.value}})}} value={formData.username}></input>
                </div>
                ...
                <button type='submit' className='bg-red-600 text-white text-2xl font-light py-3 w-full rounded-sm text-center animate-all duration-400 hover:bg-red-700 cursor-pointer' >Sign up</button>
                <p className='text-gray-400 flex justify-center text-2xl font-light '>OR</p>
                <button type="button" onClick={handleNav} className='bg-white/20 text-white text-2xl font-light py-3 w-full rounded-sm text-center animate-all duration-400 hover:bg-white/15 cursor-pointer'>Log in</button>
            </form>
        </div>
    </div>
  )
